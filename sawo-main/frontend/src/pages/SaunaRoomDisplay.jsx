...
// Display page for individual sauna room details
// Based on ProductPage.jsx structure
...
function localOrRemote(room, field) {
  return room?.[`local_${field}`] || room?.[field] || null;
}
...
function getImageUrl(room, field) {
  return resolveUrl(localOrRemote(room, field));
}
function getImagesArray(room, field) {
  const local = room?.[`local_${field}`];
  const remote = room?.[field];
  const arr = (local?.length ? local : remote) || [];
  return arr.map(resolveUrl).filter(Boolean);
}
function getFilesArray(room) {
  const local = room?.local_files;
  const remote = room?.files;
...
export default function SaunaRoomDisplay() {
  ...
  const { rooms: localRooms, loading } = useLocalSaunaRooms();

  const room = useMemo(() => {
    if (!localRooms.length) return null;
    return localRooms.find(r => r.slug === slug && r.status === "published" && r.visible !== false) || null;
  }, [localRooms, slug]);
  ...
  if (error || !room) return (
    <div style={{
      ...
      <p style={{ color: "#a67853", margin: "0 0 24px", fontStyle: "italic", fontSize: "0.88rem" }}>
        {error || "This sauna room doesn't exist or isn't published yet."}
      </p>
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", justifyContent: "center" }}>
        <Link to="/sauna-rooms" style={{ display: "inline-flex", alignItems: "center", gap: 7, padding: "9px 20px", background: "linear-gradient(135deg,#8b5e3c,#a67853)", color: "#fff", textDecoration: "none", fontWeight: 700, borderRadius: 7, fontSize: "0.82rem" }}>
          Browse Rooms
        </Link>
  ...
  const files        = getFilesArray(room);
  const images       = getImagesArray(room, 'images');
  const thumbnail    = getImageUrl(room, 'thumbnail');
  const specImages   = getImagesArray(room, 'spec_images');
      ...
      <div style={{ minHeight: "100vh", background: "#fff", fontFamily: "'Montserrat',sans-serif" }}>
        ...
        >
          ...
          >
            ...
            {/* RIGHT: Room info, Features, Spec Images — top aligned */}
            <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
              {(room.brand || room.room_type) && (
                <p style={{
                  ...
                  {[room.brand, room.room_type].filter(Boolean).join(" · ")}
                </p>
              )}
              ...
              {hasDesc && (
                <div style={{ marginBottom: hasSpecTable ? 32 : 0 }}>
                  <div
                    ...
                    dangerouslySetInnerHTML={{ __html: cleanHTMLStyles(room.description) }}
                  />
                </div>
              )}
