...
const DISPLAY_TAGS       = ["Pail Shower"];

// ─── Grouping ─────────────────────────────────────────────────────────────────
const FIXED_ORDER = ["Pails", "Ladles", "Pail Shower"];

const GROUP_KEYWORDS = {
  Pails:        ["Pail"],
  Ladles:       ["Ladle"],
  "Pail Shower": ["Pail Shower", "Shower"],
};
    ...
    <div className="relative">
      <style>{`
        ...
        .wm-search-wrap {
          ...
          background: #fff; border: 1.5px solid #e0cfc0;
            ...
            <>
              {visibleGroups.length === 0 ? (
                ...
                visibleGroups.map(group => {
                  const q = search.trim().toLowerCase();
                  const items = (groupedProducts[group] || []).filter(p =>
                    !q ||
                    p.name?.toLowerCase().includes(q) ||
                    (p.categories || []).some(c => c.toLowerCase().includes(q)) ||
                    (p.tags || []).some(t => t.toLowerCase().includes(q))
