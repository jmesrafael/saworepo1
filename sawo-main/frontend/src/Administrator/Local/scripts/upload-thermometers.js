...
/**
 * Upload thermometer product images to Supabase storage
 ...
 */
async function main() {
  console.log("🚀 Starting thermometer product upload to Supabase...\n");
  ...
  // Filter thermometer products
  const allThermometers = productsData.filter(p => p.categories && p.categories.includes("thermometers"));

  console.log(`📦 Found ${allThermometers.length} thermometer products to upload`);
  ...
  for (const product of allThermometers) {
  ...
  // Step 2: Delete old thermometer records and insert new ones
  console.log("📝 Deleting old thermometer records and inserting updated ones...\n");

  // Delete all existing thermometer records
  const { error: deleteError } = await supabase
    .from("products")
    .delete()
    .contains("categories", ["thermometers"]);

  if (deleteError) {
    console.error("⚠️  Warning: Error deleting old thermometers:", deleteError.message);
    // Continue anyway - the upsert will handle duplicates
  } else {
    console.log("  ✓ Deleted old thermometer records");
  }

  // Insert updated products
  const { data, error } = await supabase
    .from("products")
    .upsert(allThermometers, { onConflict: "slug" })
    .select("id, slug, name");
  ...
  console.log("\n🎉 Upload complete!");
  console.log(`\nTo verify in AccessoriesPage:`);
  console.log(`  1. Switch dataSource to "live"`);
  console.log(`  2. Filter by category "thermometers"`);
  console.log(`  3. All ${allThermometers.length} thermometer products should appear with images`);
}
